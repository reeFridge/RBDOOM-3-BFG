...
==================
*/
extern "C" viewEntity_t* R_SortViewEntities( viewEntity_t* vEntities )
{
		...
		for( viewLight_t* vLight = viewDef->viewLights; vLight != NULL; vLight = vLight->next )
		{
			...
			if( vLight->entityInteractionState != NULL )
			{
				...
				if( vLight->entityInteractionState[entityIndex] == viewLight_t::INTERACTION_YES )
				{
					...
					const size_t entryRow = vLight->lightDef->index;
					const size_t entryColumn = entityIndex;
					staticInteractions[numContactedLights] = ztech_renderWorld_getInteractionEntry(
							render_world_ptr,
							entryRow, entryColumn);
			...
			if( !modelIsVisible )
			{
				...
				if( lightDef->areaNum != -1 )
				{
					...
					{
						if( ztech_renderWorld_areasAreConnected( render_world_ptr, lightDef->areaNum, ref->area->areaNum, PS_BLOCK_VIEW ) )
						{
							areasConnected = true;
							break;
						}
